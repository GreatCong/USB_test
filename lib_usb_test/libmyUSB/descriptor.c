	...
	int host_endian)
{
	...
	while (size >= INTERFACE_DESC_LENGTH) {
		...
		altsetting = (struct libusb_interface_descriptor *)realloc(altsetting,
		...
		if (ifp->bNumEndpoints > 0) {
			...
			endpoint = (struct libusb_endpoint_descriptor *)malloc(tmp);
	...
	int host_endian)
{
	...
	interface = (struct libusb_interface *)malloc(tmp);
...
{
	struct libusb_config_descriptor *_config = (struct libusb_config_descriptor *)malloc(sizeof(*_config));
	unsigned char tmp[8];
	...
	uint8_t config_index, struct libusb_config_descriptor **config)
{
	...
	_config = (struct libusb_config_descriptor *)malloc(sizeof(*_config));
	if (!_config)
		return LIBUSB_ERROR_NO_MEM;
