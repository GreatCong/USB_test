...
{
	struct discovered_devs *ret =(struct discovered_devs *)
		malloc(sizeof(*ret) + (sizeof(char *) * DISCOVERED_DEVICES_SIZE_STEP));
	...
	struct discovered_devs *discdevs, struct libusb_device *dev)
{
	...
	discdevs = (struct discovered_devs *)realloc(discdevs,
		sizeof(*discdevs) + (sizeof(char *) * capacity));
	if (discdevs) {
	...
	unsigned long session_id)
{
	...
	struct libusb_device *dev = (struct libusb_device *)malloc(sizeof(*dev) + priv_size);
	int r;
	...
	libusb_device ***list)
{
	...
	len = discdevs->len;
	ret = (libusb_device **)malloc(sizeof(char *) * (len + 1));
	if (!ret) {
...
int libusb_open(libusb_device *dev, libusb_device_handle **handle)
{
	...
	_handle = (struct libusb_device_handle *)malloc(sizeof(*_handle) + priv_size);
	if (!_handle)
		return LIBUSB_ERROR_NO_MEM;
...
int libusb_init(libusb_context **context)
{
	...
	struct libusb_context *ctx = (struct libusb_context *)malloc(sizeof(*ctx));
	int r;
