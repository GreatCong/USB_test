...
static void free_iso_urbs(struct linux_transfer_priv *tpriv)
{
	...
	for (i = 0; i < tpriv->num_urbs; i++) {
		struct usbfs_urb *urb = tpriv->urbS.iso_urbs[i];
		if (!urb)
			break;
		free(urb);
	}

	free(tpriv->urbS.iso_urbs);
}
	...
	unsigned char urb_type)
{
	...
	struct linux_transfer_priv *tpriv = (struct linux_transfer_priv *)usbi_transfer_get_os_priv(itransfer);
	...
	urbs = (struct usbfs_urb *)malloc(alloc_size);
	if (!urbs)
		return LIBUSB_ERROR_NO_MEM;
	memset(urbs, 0, alloc_size);
	tpriv->urbS.urbs = urbs;
...
static int submit_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	for (i = 0; i < num_urbs; i++) {
		...
		urb = (struct usbfs_urb *)malloc(alloc_size);
		if (!urb) {
			free_iso_urbs(tpriv);
			return LIBUSB_ERROR_NO_MEM;
		}
...
static int submit_control_transfer(struct usbi_transfer *itransfer)
{
	...
	urb = (struct usbfs_urb *)malloc(sizeof(struct usbfs_urb));
	if (!urb)
		return LIBUSB_ERROR_NO_MEM;
...
static void cancel_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	tpriv->awaiting_discard = 0;
	for (i = 0; i < tpriv->num_urbs; i++) {
		int tmp = ioctl(dpriv->fd, IOCTL_USBFS_DISCARDURB, tpriv->urbS.iso_urbs[i]);
		if (tmp == 0)
			tpriv->awaiting_discard++;
		else if (errno == EINVAL)
			tpriv->awaiting_reap++;
	...
	struct usbfs_urb *urb)
{
	...
	if (urb->status == 0) {
		...
		for (i = 0; i < urb->number_of_packets; i++) {
			...
			lib_desc->status = (enum libusb_transfer_status)urb_desc->status;
